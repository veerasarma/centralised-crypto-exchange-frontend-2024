...
// import service
import { apiOrderPlace } from '../../services/perpetual/PerpetualService'
...
export default function CancelBtn({ orderInfo }: any) {
    ...
    const handleClose = () => {
        setFormValue({ ...formValue, ...{ quantity: toFixed(orderInfo?.quantity,tradePair?.baseFloatDigit), price: 0 } })
        setMShow(false)
        setLShow(false)
    };
    ...
    const handlePercentageClick = (percentage: any) => {
        ...
        setFormValue({ ...formValue, ...{ quantity: toFixed(newAmount,tradePair?.baseFloatDigit) } });
    };
    const handleChange = (
        ...
        if(name === "price" && value.split(".")[1]?.length > tradePair?.quoteFloatDigit){
            return
        }
        if(name === "quantity" && value.split(".")[1]?.length > tradePair?.baseFloatDigit){
            return
        }
    ...
    useEffect(() => {
        if (orderInfo?.quantity) {
            setFormValue({ ...formValue, ...{ quantity: toFixed(orderInfo.quantity,tradePair?.baseFloatDigit), price: toFixed(marketData?.markPrice,tradePair?.quoteFloatDigit) } });
        }
    }, [orderInfo.quantity])
