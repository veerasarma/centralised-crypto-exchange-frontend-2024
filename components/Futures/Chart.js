...
//   // redux state
//   const { tradePair } = useSelector((state) => state.perpetual);
...
//     }
//     // }
//   }, [tradePair._id, themeDataStore]);

//   // useEffect(() => {
//   //   setChartTheme(themeDataStore || "light"); // Update chartTheme when themeDataStore changes
//   // }, [themeDataStore]);
//   const binanceSymbol = `BINANCE:${tradePair.baseCoinSymbol}${tradePair.quoteCoinSymbol}`;
...
const Chart = (props) => {
  ...
  const themeDataStore = useSelector((state) => state.UserSetting.data.defaultTheme);
  ...
  useEffect(() => {
    if (!isEmpty(tradePair) && !isEmpty(tradePair._id) && !isEmpty(themeDataStore)) {
      if (tvWidget !== null) {
        tvWidget.remove();
        tvWidget = null;
      }
