...
export default function PairList(props: any) {
    ...
    const handlePair = (item: any) => {
        ...
        history.push({ pathname: '/futures/' + item.baseCoinSymbol + "_" + item.quoteCoinSymbol }, undefined, { shallow: true })
    }
    ...
    useEffect(() => {
        let favourites = favourite?.futureCount ? favourite.futureCount : []
    ...
    useEffect(() => {
        // socket
        socketContext.perpSocket.on('marketPrice', (result: any) => {
        ...
        return () => {
            socketContext.perpSocket.off("marketPrice");
        }
    }, [allMarketData])

    return (
        <div className={`${spot.market_pair_info}  ${spot.fixed} ${props.pair_box ? spot.show : ""} `}  >
            <div className={spot.box}>
                ...
                <div className={spot.pairs}>
                    <h6 className={`${currTab == 'fav' ? spot.active : ""} `}
                ...
                <div className={`${spot.table_box} ${spot.table_box_dropdown}`}>
                    ...
                    <Table >
                        <thead>
                            <tr>
                                <th>Pairs<i className="fa-solid fa-sort"></i></th>
                                <th>Price <i className="fa-solid fa-sort"></i></th>
                                <th>Change <i className="fa-solid fa-sort"></i></th>
                            </tr>
                        </thead>

                        <tbody>
                            {
                                // allMarketData?.length > 0 && allMarketData.map((item: any, index: number) => {
                                !loading && pairDet?.length > 0 && pairDet.map((item: any, index: number) => {
                                    {
                                        if (search && currTab == 'fav' ? favPair.includes(item._id) && (searchVal?.toUpperCase() === item?.quoteCoinSymbol?.toUpperCase() || searchVal?.toUpperCase() === item?.baseCoinSymbol?.toUpperCase()) : searchVal?.toUpperCase() === item?.baseCoinSymbol?.toUpperCase() || searchVal?.toUpperCase() === item?.quoteCoinSymbol?.toUpperCase()) {
                                            if (currTab == 'fav') {
                                                                ...
                                                                <td >
                                                                    {
                                                                        favPair.includes(item._id) ?
                                                                            <i className={`fa-solid fa-bookmark`} style={FavColor} onClick={() => handleFav(item._id)}></i> :
                                                                            <i className={`fa-solid fa-bookmark`} onClick={() => handleFav(item._id)} ></i>
                                                                    }
                                                                    {item.baseCoinSymbol}/{item.quoteCoinSymbol}</td>
                                                                <td > {toFixed(item.markPrice, item.quoteFloatDigit)}</td>
                                                                {
                                                                    isVol ? <td > {toFixed(item.secondVolume, item.quoteFloatDigit)}</td> : <td className={item.change < 0 ? spot.red : spot.green} >{!isEmpty(item.change) ? toFixed(item.change, item.quoteFloatDigit) : 0}%</td>
                                                                }
                                                            </tr>
                                                        </>
                                                    )
