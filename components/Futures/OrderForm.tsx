...
export default function OrderForm({ posCount }: any) {
  ...
  const { derivativeMode, leverage } = useSelector(
    (state) => state.UserSetting?.data?.mode
  );
        ...
        <div className={spot.orderform_navtab}>
          ...
          <Tab.Container defaultActiveKey="open">
            <div className={`pt-0 ${spot.ordertab}`}>
              ...
              >
                <Nav.Item>
                  <Nav.Link eventKey="open" onClick={() => setOrderStatus("open")}>Open</Nav.Link>
                </Nav.Item>
                <Nav.Item>
                  <Nav.Link eventKey="close" onClick={() => setOrderStatus("closed")}>Close</Nav.Link>
                </Nav.Item>
              </Nav>
            </div>
            <Tab.Content>
                      ...
                      {derivativeMode == "cross" &&
                        <>
                          <div className={spot.right_top_future}>
                            Available:{" "}
                            {parseFloat(walletBal?.availableBal) >= 0 && (
                              <span style={green}>
                                {truncateDecimals(
                                  parseFloat(walletBal?.availableBal) - parseFloat(walletBal?.locked) +
                                  parseFloat(totalPnL),
