...
export default function PairList(props: any) {
  ...
  useEffect(() => {
    // socket
    socketContext.perpSocket.on("usdtPerFundingRate", (result: any) => {
      let tempPairList = [...allMarketData];
      let pairIndex = tempPairList.findIndex((el: any) => {
        return el._id == result.pairId;
      });
      ...
      <div className={spot.box}>
        ...
        <div className={spot.right_top}>
          ...
          <ul className={`${spot.feeDetails}`}>
            <li><label>Taker</label> <span>({tradePair?.takerFee}%)</span></li>
            <li><label>Maker</label> <span>({tradePair?.makerFee}%)</span></li>
          </ul>
        </div>
      </div>
    </div>
