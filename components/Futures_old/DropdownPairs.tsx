...
export default function PairList(props: any) {
    ...
    const { marketData, favPair, allMarketData } = useSelector((state: any) => state.perpetual);
    ...
    return (
        <div className={`${spot.market_pair_info}  ${spot.fixed} ${props.pair_box ? spot.show : ""} `}  >
            <div className={spot.box}>
                ...
                <div className={`${spot.table_box} h-100`}>
                                                    ...
                                                    return (
                                                        <>
                                                            <tr onClick={() => handlePair(item)} key={index} >
                                                                ...
                                                                <td > {toFixedDown(item.markPrice, item.quoteFloatDigit)}</td>
                                                                {
                                                                    isVol ? <td > {toFixedDown(item.secondVolume, item.quoteFloatDigit)}</td> : <td className={item.change < 0 ? spot.red : spot.green} >{!isEmpty(item.change) ? toFixedDown(item.change, item.quoteFloatDigit) : 0}%</td>
                                                                }
                                                            </tr>
                                                        </>
                                                    )
                                                    ...
                                                    <>
                                                        <tr onClick={() => handlePair(item)} key={index} >
                                                            <td >
                                                                ...
                                                                {item.baseCoinSymbol}/{item.quoteCoinSymbol}</td>
                                                            <td > {toFixedDown(item.markPrice, item.secondFloatDigit)}</td>
                                                            {
                                                                isVol ? <td > {toFixedDown(item.secondVolume, item.quoteFloatDigit)}</td> : <td className={item.change < 0 ? spot.red : spot.green} >{!isEmpty(item.change) ? toFixedDown(item.change, item.quoteFloatDigit) : 0}%</td>
                                                            }
                                                        </tr>
