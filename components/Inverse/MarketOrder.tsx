...
export default function MarketOrder() {
  ...
  const [formValue, setFormValue] =
    useState<MarketFormValues>(initialValue);
  ...
  const handleChange = (
    ...
    if (name == "quantity") {
      setFormValue({ ...formValue, ...{ [name]: value, ["orderCost"]: (value / marketData.markPrice) / inverseLeverage } });
    } else {
  ...
  const hPercClick = (perc: any, perStatus: any) => {
    ...
    let qty = (ordCos * marketData.markPrice) / inverseLeverage // ordCos * inverseLeverage
    setFormValue({
      ...formValue,
      ...{
        ["quantity"]: toFixed(
  ...
  useEffect(() => {
    if (percent > 0) {
      ...
      setFormValue({ ...formValue, ...{ ["orderCost"]: (formValue.quantity / marketData.markPrice) / inverseLeverage, leverage: inverseLeverage } });
    }
  }, [inverseLeverage]);
  return (
    <>
      ...
      >
        ...
        <Modal.Body>
          <div className={styles.modalContent}>
            ...
            <div className={styles.modalRow}>
              <div>Leverage</div>
              <div className={styles.modalOrder}>{`${capitalize(inverseMode)} ${inverseLeverage}x`}</div>
            </div>
          </div>
