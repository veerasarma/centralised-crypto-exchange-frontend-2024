...
//improt service
import { getOpenOrder } from "../../services/inverse/InverseService";
//import component
...
export default function OpenOrder({ countRef }: any) {
  const { showOInverse } = useSelector(
  ...
  const totalCount = (data: any) => {
    try {
      ...
      for (let item of data) {
        if (showOInverse || tradePair._id.toString() == item.pairId.toString()) {
          count += 1
        }
      }
  ...
  useEffect(() => {
    if (!isEmpty(tradePair._id) && isLogin && !isEmpty(currentPage)) {
      ...
      // socket
      socketContext.inverSocket.on("inverseOpenOrder", (result: any) => {
        console.log(showOInverse, '----------126', result, tradePair)
        if ((!showOInverse && result.pairId == tradePair._id) || showOInverse) {
          totalCount(result.data)
          setOrderData({
            currentPage: result.currentPage,
            nextPage: result.nextPage,
      ...
      return () => {
        socketContext.inverSocket.off("inverseOpenOrder");
      };
    }
  }, [tradePair._id, isLogin, showOInverse]);
  return (
                ...
                if (showOInverse || tradePair._id.toString() == item.pairId.toString()) {
                  return (
                    <tr key={index}>
                      ...
                      <td>
                        {
                          item?.openStatus == "cOpen" ? "-" :
                            item.status == "open" && item?.tpPrice > 0 ? (
                              <span>
                                {item?.tpPrice}
                      ...
                      <td>
                        {
                          item?.openStatus == "cOpen" ? "-" : item.status == "open" && item?.slPrice > 0 ? (
                            <span>
                              {item?.slPrice} {/*  {item.slPrice > 0 ? item.slPrice : "Add"}
                              &nbsp;
                               <i
                                onClick={() => {
                                  slEditShow();
