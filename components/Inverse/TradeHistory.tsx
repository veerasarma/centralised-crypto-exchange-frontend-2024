...
//improt service
import { getTradeHistory } from "../../services/inverse/InverseService";
...
export default function TradeHistory() {
  ...
  useEffect(() => {
    if (!isEmpty(tradePair._id) && isLogin && !isEmpty(currentPage)) {
      ...
      // socket
      socketContext.inverSocket.on("inverseTradeHistory", (result: any) => {
      ...
      return () => {
        socketContext.inverSocket.off("inverseTradeHistory");
      };
    }
  }, [tradePair, isLogin]);
              ...
              data.map((item: any, index: number) => {
                return (
                  <tr key={index}>
                    ...
                    <td>{item.orderType ? capitalize(item.orderType) : "-"}</td>
                    <td>
                      {" "}
                      {truncateDecimals(
