...
const TopVolume = ({ pairList, spotMark }: any) => {
    // redux
        ...
        pairList?.length > 0 &&
            ...
            .map((item: any) => {
                ...
                let data = {
                    ...
                    Volume: !isEmpty(item?.secondVolume) ? toFixed(item.secondVolume, 2) : 0,
                ...
                el.map((item: any) => {
                    if (item._id == spotMark.pairId) {
                        ...
                        sockList.push({
                            ...
                            Volume: !isEmpty(spotMark.data?.secondVolume) ? toFixed(spotMark.data.secondVolume, 2) : 0,
                        });
                    } else {
                        sockList.push(item);
                    }
            ...
            <div>
                {data && data.length > 0 && data
                    ...
                    .map((item: any, key: any) => (
                        <a href={`/spot/${item.firstCurrencySymbol}_${item.secondCurrencySymbol}`} className={styles.dflex} key={key} target="_blank">
                            <div>
                                <Image
                            ...
                            <div>
                                <p>${Humanize.compactInteger(item.Volume)}</p>
                            </div>
                            <div>
