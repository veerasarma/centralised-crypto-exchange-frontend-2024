...
export default function PairList(props: any) {
  ...
  const { marketData, allMarketData } = useSelector(
  ...
  const [currTab, setCurrTab] = useState<string>("spot");
  const [searchVal, setSearchVal] = useState<string>("");
  const [search, setSearch] = useState<boolean>(false);
  const [favPair, setFavourite] = useState<array>(fav);
  ...
  useEffect(() => {
    ...
    socketContext.spotSocket.on("marketPrice", (result: any) => {
      console.log(result.pairId, "--------92");
      console.log(marketData._id, "--------93");
    ...
    >
      <div className={spot.box}>
        ...
        <div className={spot.pairs}>
          ...
          <h6
            className={`${currTab == "spot" ? spot.active : ""} `}
            onClick={() => pair_tab("spot")}
          >
            Spot
          </h6>
        </div>
          ...
          <Table>
            ...
            <tbody>
              {allMarketData?.length > 0 &&
                allMarketData.map((item: any, index: number) => {
                  {
                    if (
                      search && currTab == "fav"
