...
export const securityLevel = (item) => {
  if (
    item?.idProof == "approved" &&
    item?.twoFAStatus == "enabled" &&
    item?.emailStatus == "verified" &&
    item?.phoneStatus == "verified" &&
    item.assetPasswordStatus == true &&
    item.antiphishingStatus == true
  ) {
    return "High";
  } else if (
    item?.idProof == "approved" &&
